...
if(!empty($_SESSION['username'])){      
        $sqlproducts = "SELECT t1.itemId, t2.code, t2.company, t1.productName, t1.prodId, t1.customer, t1.quantity, t1.unitPrice, t1.totalPrice, t2.image, t2.id, t1.Date FROM cart as t1, products as t2  WHERE t1.customer='{$_SESSION['username']}' AND (t2.id = t1.prodId) ORDER BY t1.Date DESC";
        $query = mysqli_query($con, $sqlproducts); 
        if ($query -> num_rows > 0){ ?>
        <table class="all-tbls" cellpadding="5" cellspacing="1" >
            <tbody> 
		    <nav class="top">
            <h3  class="txt-heading-top">MY SHOPPING CART</h3>
		    </nav>
            <tr>  
